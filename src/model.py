...
class Model:
    ...
    def train(self):
        ...
        """
        This is where the model is trained. metaboloc contains 3 base classifiers and a final ensemble model that is
    ...
    def predict(self, data=None, write=True, p_out=None, d_out=None, pred_idx=None):
        ...
        :param d_out: Optional, String Prediction result directory path
        ...
        if data.p_data and os.path.isfile(data.p_data):
            ...
            # Write results
            if write:
                p_out = self.write_predictions(predictions=predictions, evaluations=evaluations, data=data,
    ...
    def clfs_predict(self, nidxs_target, data=None, to_eval=False, fill=True, nidxs_train=None, eval_idx=None):
        ...
        for istep in range(n_steps):
            ...
            print('\n[ Step %d/%d ]  %d nodes / %d expandable links' % (istep+1, n_steps, len(path[istep]['nidxs']),
                                                                        len(path[istep]['links'])))

            # Predict
            predictions = self.clf_all_predict(nidx_target=nidxs, data=self.datas[blend_idx])

            # Baseline
            y_bl_pred = self.bl_predict(n_samples=len(nidxs), data=self.datas[blend_idx])
            predictions['ybl'] = y_bl_pred
    ...
    def _print_eval(self, predictions, to_eval=False):
        ...
        if self.verbose:
            if to_eval:
                ...
                # baseline
                r = self.scores(y=predictions['ytruth'], y_pred=predictions['ybl'])
                coverage = self.calc_coverage(predictions['ybl'])
                print('| {:<5}  {:<7.4f}  {:<6.4f}  {:<9.4f}  {:<6.4f}  {:>8.1%} |'.format(
                    'bl', r['aucroc'], r['f1'], r['precision'], r['recall'], coverage))
                ...
                # baseline
                coverage = self.calc_coverage(predictions['ybl'])
                print('| {:<5}  {:>8.1%} |'.format('bl', coverage))
    ...
    def blend_data(self, idxs=None):
        ...
        for i in idxs:
            ...
            datax.nidx2layer += [k + '-%d'%i for k in self.datas[i].nidx2layer] \
                if self.datas[i].nidx2layer and self.train_multilayers \
                else [str(i) for _ in range(len(self.datas[i].node_labels))]
    ...
    def bl_predict(self, n_samples, data=None):
        ...
        return self._predict_proba(bl, np.random.rand(n_samples, 1))
    ...
    def _opt_round_cutoff(self, x, y, ypred, ybl):

        self.round_cutoff = x

        r = self.scores(y, ypred)
        rbl = self.scores(y, ybl)
        c = self.calc_coverage(ypred)

        if 'hard' not in self.aim:
            return -(r[self.aim] / rbl[self.aim]) * (r['f1'] / rbl['f1']) * c * np.ceil(x)
        else:
            aim = self.aim.replace('hard', '')
            return -(r[aim] / rbl[aim]) * c * np.ceil(x)
...
class Data:
    ...
    def __init__(self, p_data='', exclude_links=None, labels=None, k_neighbors=3, min_network_size=10,
        ...
        metaboloc-Data (TDF) format (see metaboloc-Data format in metaboloc.py for detail).
